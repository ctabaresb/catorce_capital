...
def _ensure_summary_schema(item: dict):
    """
    Harden response schema for clients:
    Ensure item['result']['summary'] contains:
      - has_alpha (default False)
      - why (default "")
      - why_no_alpha (default [])
    This does NOT write back to DynamoDB; it only mutates the in-memory response.
    """
    try:
        ...
        if "has_alpha" not in summary:
            summary["has_alpha"] = False
        if "why" not in summary:
            summary["why"] = ""
        if "why_no_alpha" not in summary:
            summary["why_no_alpha"] = []
    except Exception:
        # Never break /status due to schema hardening
        return
...
def handler(event, context):
    try:
        ...
        # Harden schema for client response (no DB writes)
        _ensure_summary_schema(item)
