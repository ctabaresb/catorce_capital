#!/usr/bin/env python3
"""
train_xgb_mfe_v2_static.py

MFE 2.0 — Corrected Spread Static Classifier
================================================

SAME proven architecture as train_xgb_mfe.py (Hyperopt, isotonic calibration,
take-profit exit simulation) with ONE critical fix:

The minute REST API captures stale book snapshots where:
  - best_bid is $10 LOWER than real (72.6% of minutes)
  - best_ask is $11 HIGHER than real (73.9% of minutes)
  - apparent spread: 4.65 bps. Real HFT spread: 1.56 bps.
  - Mid price is correct (+0.07 bps, corr 0.99990)

FIX: Replace bid/ask with mid ± half_spread in ALL computations.
  - Entry: mid_t + half_spread (instead of ask_t)
  - Future exits: future_mid - half_spread (instead of future_bid)
  - TP hit: future_mid - half_spread > entry + TP
  - P2P: (end_mid - half_spread) / entry - 1

Everything else is IDENTICAL to the proven script.

Usage:
    python strategies/train_xgb_mfe_v2_static.py \
        --parquet data/artifacts_xgb/xgb_features_bitso_btc_usd_180d.parquet \
        --horizon 5 --spread_cost 2.0 --max_evals 30

    python strategies/train_xgb_mfe_v2_static.py \
        --parquet data/artifacts_xgb/xgb_features_bitso_btc_usd_180d.parquet \
        --horizon 10 --spread_cost 2.0 --max_evals 30
"""
...
def compute_mfe_targets(df: pd.DataFrame, horizon: int,
                         tp_levels_bps: list = None,
                         spread_cost_bps: float = 2.0) -> pd.DataFrame:
    """
    Compute MFE using MID prices (validated correct) + calibrated spread cost.

    Entry = mid_t + half_spread
    Future exit prices = future_mid - half_spread
    MFE = max(future_exit_prices)

    Take-profit simulation:
      For each TP level, find FIRST minute where exit_price > entry + TP.
      If found: exit at that minute's exit_price. 
      If not: time-exit at exit_price_{t+N}.
    """
    ...
    # Use mid_bbo if available (may be more precise)
    ...
    half_spread = spread_cost_bps / 2.0 / 1e4  # as fraction

    # Entry price: mid + half spread
    entry = mid * (1.0 + half_spread)

    print(f"  Computing MFE targets (horizon={horizon}m, TP levels={tp_levels_bps} bps, "
          f"spread_cost={spread_cost_bps:.1f} bps)...")
    print(f"  Using MID-BASED prices (validated correct, +0.07 bps vs HFT)")
    print(f"  Entry = mid + {spread_cost_bps/2:.2f} bps  |  Exit = mid - {spread_cost_bps/2:.2f} bps")

    # ── Build matrix of future EXIT prices (mid - half_spread) ─────────
    future_exits = np.full((n, horizon), np.nan)
    future_mids  = np.full((n, horizon), np.nan)
    for k in range(1, horizon + 1):
        shifted_mid = np.empty(n)
        shifted_mid[:n-k] = mid[k:]
        shifted_mid[n-k:] = np.nan
        future_mids[:, k-1] = shifted_mid
        future_exits[:, k-1] = shifted_mid * (1.0 - half_spread)

    mfe_exit = np.nanmax(future_exits, axis=1)   # best exit price in window
    mae_exit = np.nanmin(future_exits, axis=1)    # worst exit price in window
    end_exit = future_exits[:, -1]                 # exit price at t+N
    ...
    d[f"mae_ret_{horizon}m_bps"] = (mae_exit / (entry + 1e-12) - 1.0) * 1e4

    # Point-to-point return at horizon
    d[f"p2p_ret_{horizon}m_bps"] = (end_exit / (entry + 1e-12) - 1.0) * 1e4

    # Also store raw mid-to-mid for comparison
    ...
    for tp in tp_levels_bps:
        tp_price = entry * (1.0 + tp / 1e4)  # take-profit price level above entry
    ...
    for tp in tp_levels_bps:
        ...
        # Find first touch: first minute where exit price > tp_price
        ...
        # Exit price: at first touch or at end
    ...
    # ── Also store old-style MM returns for comparison ─────────────────────
...
def main():
    ...
    ap.add_argument("--horizon", type=int, default=10, choices=[1, 2, 3, 5, 7, 10])
    ...
    ap.add_argument("--spread_cost", type=float, default=2.0,
                    help="Spread cost in bps (HFT-calibrated, default=2.0)")
    ...
    OUT     = os.path.join(args.out_dir, f"{asset}_{H}m_tp{TP}_sc{args.spread_cost:.0f}")
    ...
    mlflow.set_experiment(f"mfe_{asset}_{H}m_tp{TP}_sc{args.spread_cost:.0f}")

    print(f"\n{'#'*80}")
    print(f"  MFE 2.0 — CORRECTED SPREAD (static + Hyperopt)")
    print(f"  Asset: {asset}  |  Horizon: {H}m  |  TP: {TP}bp  |  Evals: {args.max_evals}")
    print(f"  Spread cost: {args.spread_cost:.1f} bps  |  Entry: mid+{args.spread_cost/2:.1f}  Exit: mid-{args.spread_cost/2:.1f}")
    print(f"{'#'*80}")
    ...
    df = compute_mfe_targets(df, H, TP_LEVELS, spread_cost_bps=args.spread_cost)
