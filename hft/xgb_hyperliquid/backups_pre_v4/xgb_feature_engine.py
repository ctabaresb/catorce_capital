...
class XGBFeatureEngine:
    ...
    def tick(self, hl_snapshot, cross_mids: Dict[str, float] = None):
        ...
        if bn:
            ...
            # Carry forward previous values to avoid feature degradation on transient failures
            _prev = self._buffer[-1]
            bar.bn_mid = _prev.bn_mid
            bar.bn_close = _prev.bn_close
            bar.bn_volume = _prev.bn_volume
            bar.bn_n_trades = _prev.bn_n_trades
            bar.bn_taker_buy_vol = _prev.bn_taker_buy_vol
            bar.bn_quote_vol = _prev.bn_quote_vol
            logger.warning(f"{getattr(self, 'coin', '?')}: Binance fetch failed, carrying forward")
